...
with app.app_context():
    ...
    Address(
        street="Dietrichstraße", housenumber="37b", postalcode="33104", city="Paderborn"
    ).insert()
    ...
    EventAttendees(
        eventId=db.session.query(Events).filter(Events.date == "2024-03-22").one().id,
    ...
    EventAttendees(
        eventId=db.session.query(Events)
        .filter(Events.date == datetime.today().date())
        .one()
        .id,
